...
本章整理评估模型时常用的概念：数据划分、交叉验证、评估指标、ROC 曲线、模型选择、欠拟合、过拟合、偏差、方差、学习曲线和数据泄漏。
...
需要注意，交叉验证通常只在训练集内部进行。测试集仍然要保留到最后一次性使用。
...
ROC-AUC 关注排序能力，不依赖某一个固定阈值。但在类别极度不平衡时，PR 曲线有时更有参考价值，因为 PR 曲线更关注正类识别效果。
...
偏差描述模型的平均预测和真实规律之间的差距。高偏差通常说明模型假设太简单，无法表达数据中的真实规律，因此训练集和验证集都表现不好。

方差描述模型对训练数据扰动的敏感程度。高方差通常说明模型太容易受训练样本中的偶然性影响，训练集表现很好，但换到验证集就明显变差。
...
偏差和方差不是简单地“一个好一个坏”。模型太简单时偏差高；模型太复杂时方差高。训练模型时经常是在两者之间寻找平衡。

正则化主要是降低方差：它限制模型不要过度追逐训练集细节。但正则化过强会提高偏差，使模型变得太保守。
