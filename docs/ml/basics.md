...
机器学习可以按不同角度分类。不同分类角度回答的问题不一样，学习时不要混在一起。
...
| 类型 | 数据特点 | 典型任务 |
| --- | --- | --- |
| 监督学习 | 有标签，有正确答案 | 回归、分类 |
| 无监督学习 | 无标签，没有提前给出正确答案 | 聚类、降维、密度估计 |
| 半监督学习 | 只有一部分样本有标签 | 小样本分类、医学图像标注 |
| 自监督学习 | 标签从数据自身构造 | 语言模型、图像表征学习 |
| 强化学习 | 通过环境反馈获得奖励 | 游戏智能体、机器人控制 |

另一种常见角度是按模型如何建模来分：

```text
模型建模方式
├─ 判别式模型：直接学习输入到输出的映射，或者学习类别边界
└─ 生成式模型：学习数据是如何产生的，再用这个过程做判断或生成
```
...
| 类型 | 关注点 | 典型例子 |
| --- | --- | --- |
| 判别式模型 | 类别之间如何区分 | 逻辑回归、支持向量机、普通神经网络分类器 |
| 生成式模型 | 数据是如何产生的 | 朴素贝叶斯、高斯混合模型、VAE、扩散模型 |

这两种分类角度不要混在同一层级。监督学习 / 无监督学习说的是“有没有标签”；生成式 / 判别式说的是“模型怎么学”。例如分类问题通常属于监督学习，但分类模型本身既可以是生成式，也可以是判别式。

判别式模型更关心决策边界。比如判断肿瘤良性或恶性时，逻辑回归直接学习“什么样的特征更像恶性”。生成式模型更关心数据分布。比如先学习良性肿瘤和恶性肿瘤各自的特征分布，再比较一个新样本更可能来自哪一类。

深度学习可以理解为机器学习中的一类方法，重点是用多层神经网络自动学习特征表示。传统机器学习通常需要人工设计特征，例如房屋面积、房间数、肿瘤大小；深度学习更擅长从图像、文本、语音这类复杂数据中自动提取表示。卷积神经网络、循环神经网络、Transformer、图神经网络都属于这种思路下的模型结构。

考试或面试里常见的判断方式是：

| 问法 | 判断重点 |
| --- | --- |
| 是否有标签 | 有标签多半是监督学习；无标签多半是无监督学习 |
| 输出是数值还是类别 | 连续值是回归；离散类别是分类 |
| 是否学习数据生成过程 | 是则偏生成式模型 |
| 是否直接学习分类边界 | 是则偏判别式模型 |
| 是否通过奖励改进行为 | 是则属于强化学习 |
...
这个任务属于回归问题。回归不是指“模型一定是直线”，而是指输出目标是连续数值。线性回归、多项式回归、神经网络回归都可以用来解决回归问题。
...
这个任务属于分类问题。分类的输出是有限个类别，模型学习的是不同类别之间的边界，或者学习每个类别的概率。
...
| --- | --- |
| 特征 | 输入变量，例如房屋面积、肿瘤大小 |
| 标签 | 正确答案，例如实际房价、良性或恶性 |

训练模型时，算法从特征和标签之间的关系中学习规律。预测新样本时，只给模型特征，模型输出预测结果。
