...
- θ<sub>0</sub> 是截距。
- θ<sub>1</sub> 是斜率。
- h<sub>θ</sub>(x) 是模型对输入 x 的预测结果。

因为模型只有一个特征变量，所以叫单变量线性回归。

## 2.2 代价函数

线性回归要做的事情是选择合适的参数 θ<sub>0</sub> 和 θ<sub>1</sub>，让直线尽量贴合训练数据。
...
先看简化情况：假设 θ<sub>0</sub> = 0，模型变成：

$$
h_\theta(x) = \theta_1 x
$$

此时只有一个参数 θ<sub>1</sub>。不同的 θ<sub>1</sub> 会得到不同斜率的直线，也会对应不同的代价函数值 J(θ<sub>1</sub>)。

直观理解：

- 直线越贴近训练样本，预测误差越小。
- 预测误差越小，代价函数 J 越小。
- 最好的 θ<sub>1</sub> 是让 J(θ<sub>1</sub>) 最小的值。
...
这时参数有两个：θ<sub>0</sub> 和 θ<sub>1</sub>。代价函数是：
...
- α 是学习率。
- dJ / dθ<sub>j</sub> 是代价函数对参数 θ<sub>j</sub> 的偏导数。
- `j` 表示要更新的参数编号。

对 θ<sub>0</sub> 和 θ<sub>1</sub> 更新时，必须同时更新。也就是说，先用旧参数计算出两个新值，再一起赋值。
...
不要先更新 θ<sub>0</sub>，再拿新的 θ<sub>0</sub> 去计算 θ<sub>1</sub>。
...
即使 α 固定不变，接近最低点时，导数会越来越小，所以每次更新的幅度也会自然变小。
...
并且 θ<sub>0</sub> 和 θ<sub>1</sub> 仍然要同步更新。
