...
异常检测可以理解为密度估计：先学习正常样本大致分布在哪里，再把概率很低的样本判为异常。

阈值 $\epsilon$ 决定模型有多敏感：

| 阈值 | 效果 |
| --- | --- |
| $\epsilon$ 较大 | 更容易判为异常，召回率可能更高 |
| $\epsilon$ 较小 | 更谨慎，误报可能更少 |

如果有少量带标签的异常样本，应该用验证集选择 $\epsilon$，而不是凭感觉手动定。

## 10.10 高维高斯的直觉

在高维空间里，高斯分布的大部分概率质量不一定集中在均值点本身，而可能集中在离均值一定距离的薄壳区域。这和低维直觉不同。

实际做异常检测时，不要只用“离均值近不近”做判断，还要考虑特征尺度、相关性和整体概率密度。

如果异常样本数量足够多，并且类型比较固定，监督学习二分类往往比异常检测更合适。如果异常很少、类型不断变化，异常检测更自然。
