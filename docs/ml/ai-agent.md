# 第 20 章 AI 应用系统：Dify 和 Agent

Dify 是一个用于构建大模型应用的平台。它把模型调用、提示词、知识库、工作流（Workflow）、工具调用和应用发布整合到一起，适合快速搭建聊天助手、知识库问答、工作流应用和 Agent 应用。

Agent 通常译作智能体，是大模型应用的一种形态。普通聊天助手主要回答问题，Agent 更强调“闭环执行”：能理解目标、拆解任务、调用工具、观察结果，并根据反馈继续行动。
...
工具调用必须考虑权限控制。凡是会修改数据、发消息、下单、删除文件的工具，都应该加入权限校验、日志记录和人工确认。
...
RAG 在生成回答前先检索外部资料，再把检索结果作为上下文交给模型。它可以提高知识时效性，也能减少模型凭空生成内容的风险。

## 20.9 Embedding 和 Reranker

Embedding 指文本向量化模型，Reranker 指重排模型。二者常和 RAG 一起使用。
