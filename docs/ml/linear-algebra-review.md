...
矩阵元素通常写作 A<sub>ij</sub>，表示矩阵 `A` 中第 `i` 行、第 `j` 列的元素。
...
课程中一般使用 `1` 索引，也就是第一个元素写作 y<sub>1</sub>，而不是 y<sub>0</sub>。
...
如果矩阵 A 是一个方阵，并且存在逆矩阵，那么它的逆矩阵记作 A<sup>-1</sup>。
...
设 A 是一个 m x n 矩阵，它的转置记作 A<sup>T</sup>，转置后的矩阵维度会变成 n x m。
